...
for rho2exp = log10(1.2e2) %linspace(-5, 5, ceil(sqrt(N)))
for dths = 1.5 * pi %linspace(0, 2 * pi, ceil(sqrt(N)))
...
% Parameters
xi = rho1 / rho2;
m = sqrt((xi * cos(dth) - 1) ^ 2 + (xi * sin(dth)) ^ 2);
q  = (1 - xi) / m;

% Angles
phi = atan2((xi * sin(dth)), (xi * cos(dth) - 1));
...
ee = exp(linspace(log(abs(q)), log(eM), 5001));

th0_1e = @(e) -phi - acos(q./ e);
th0_2e = @(e) -phi + acos(q./ e);
...
plot(ee, th0_1e(ee), 'b')
hold on
plot(ee, th0_1e(ee)+dth, 'c')
plot(ee, -real(acos(-1./ ee)), 'm')
plot(ee, real(acos(-1./ ee)), 'r')
plot([0, eM], [-pi, -pi], '--', 'color', [0.5, 0.5, 0.5])
plot([0, eM], [pi, pi], '--', 'color', [0.5, 0.5, 0.5])
plot(ee, sign(rho1*(1+ee.*cos(th0_1e(ee)))), 'y')
plot(ee, sign(rho2*(1+ee.*cos(th0_1e(ee)+dth))), 'g')
hold off

figure(2)
plot(ee, th0_2e(ee), 'b')
hold on
plot(ee, th0_2e(ee)+dth, 'c')
plot(ee, -real(acos(-1./ ee)), 'm')
plot(ee, real(acos(-1./ ee)), 'r')
plot([0, eM], [-pi, -pi], '--', 'color', [0.5, 0.5, 0.5])
plot([0, eM], [pi, pi], '--', 'color', [0.5, 0.5, 0.5])
plot(ee, sign(rho1*(1+ee.*cos(th0_2e(ee)))), 'y')
plot(ee, sign(rho2*(1+ee.*cos(th0_2e(ee)+dth))), 'g')
hold off
